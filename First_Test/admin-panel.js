...
function createAdminPanel() {
    ...
    console.log('Admin panel UI created');
}
...
function setupAdminEvents() {
    ...
    console.log('Admin panel events setup complete');
}
...
export function showAddResourceModal() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            console.log('Resource added:', newResourceRef.key);
            overlay.remove();
            loadResources();
        ...
        if (resourceData && resourceData.url && resourceData.url.includes('firebasestorage.googleapis.com')) {
            try {
                ...
                if (storagePath) {
                    ...
                    console.log('Could not determine storage path from URL; skipping storage deletion.');
                }
            } catch (err) {
...
window.testAdminStatus = async function() {
    const { getCurrentUserId } = await import('./auth-guard.js');
    const userId = getCurrentUserId();
    console.log('Testing admin status for user:', userId);
    ...
    if (isAdmin) {
        ...
        console.log('Please add role: "admin" to user in Firebase');
    }
};

console.log('Type testAdminStatus() in console to debug');
