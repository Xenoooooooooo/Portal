import { injectNavIcons, themeManager } from './utils.js';
import { handleLogout, getCurrentUserId } from './auth.js';
import { initializeTaskManager, loadDashboard } from './task-dashboard.js';
import { initializeAdminPanel } from './admin-panel.js';
import { initializeResourcesViewer } from './resources-viewer.js';
import { initializeChatManager } from './user-features.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // If auth was already ready (auth event fired before this listener attached), initialize immediately
    try {
        const existingUid = getCurrentUserId();
        if (existingUid) {
            loadDashboard();
            initializeTaskManager();
        }
    } catch (err) {
        console.error('Error during immediate auth check:', err);
    }
...
function navigateToSection(sectionId) {
    ...
    if (targetSection) {
        ...
        if (sectionId === 'profile') {
            ...
            import('./user-features.js').then(module => {
